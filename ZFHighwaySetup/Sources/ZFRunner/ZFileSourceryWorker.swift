...
public class ZFileSourceryWorker: ZFileSourceryWorkerProtocol, AutoGenerateProtocol
{
    ...
    ) throws
    {
        ...
        self.dump = dump
        sourceryBuilder = try sourceryBuilderType.init(terminalWorker: terminal, disk: dependencies, signPost: signPost, systemExecutableProvider: systemExecutableProvider)
        self.queue = queue
    ...
    public func attemptCreateWorkers() throws
    {
        ...
        let sourcerySequence = try dump.products.filter { !$0.name.hasSuffix("Mock") }.map
        { product in
            try Sourcery(
        ...
        workers = try sourcerySequence.map { try SourceryWorker(sourcery: $0, terminalWorker: terminal, signPost: signPost, queue: queue) }
    }
